	...
	<div class="text-center">
		<ul class="pagination" style="margin:0 auto;">
			<c:if test="${startPage > pageBlock}">
				...
				<li> <a href="javascript:list_complete_pay_table('${startPage - pageBlock}')">◀</a> </li> <!-- 이전 블록으로 이동 -->
			</c:if>
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				...
				<c:if test="${i != currentPage}">
					<li> <a href="javascript:list_complete_pay_table('${i}')"> ${i}</a> </li>
				</c:if>
			</c:forEach>
			<c:if test="${pageCount > endPage}">
				<li> <a href="javascript:list_complete_pay_table('${startPage + pageBlock}')">▶</a> </li> <!-- 다음 블록으로 이동 -->
				<li> <a href="javascript:list_complete_pay_table('${pageCount}')">▶▶</a> </li> <!-- 마지막 페이지로 이동 -->
			</c:if>
		</ul>
	</div>
	<br>
