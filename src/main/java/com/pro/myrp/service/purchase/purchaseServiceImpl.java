...
public class purchaseServiceImpl implements purchaseService,CodeMyRP {
	...
	public void search_status_purchase_table_service(Model model) {
		...
		System.out.println("  -> search_status_purchase_table_service " );
		...
		if( !req.getParameter("account").equals("null") && !req.getParameter("account").equals("0") ){
			...
			System.out.println("  -> Date Search : Option");
			date_check = 2;
		}
		...
		if(cnt > 0) {
			...
			// 옵션 검색
				...
				dtos = dao.select_serch_option_status_purchase(optionMap);
				model.addAttribute("PurchaseDTOs", dtos);
		...
		if(cnt > 0) {
			...
			model.addAttribute("pageCount", pageCount);
			model.addAttribute("currentPage", currentPage);
			model.addAttribute("check",date_check);
		}
	...
	public void search_status_purchase_detail_service(Model model) {
		...
		ArrayList<SalesDTO> dtos = dao.select_detail_status_purchase(daoMap);
